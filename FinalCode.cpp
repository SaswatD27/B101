	...
	while(mcrb.BER!=0)
	{
		...
		for (int i = mutarate; i > 0; i--)
		{
			...
			if(Biome[ct[0]][ct[1]].BCR<=0)
			{
				...
				cout<<endl<<"Arrived at "<<Biome[ct[0]][ct[1]].name;
			}
			sleep (2.5);
		}
		if (mutarate == 0)
		{
			...
			switch (mutanum)
			{
				...
				cout << "It is the mutation of " << mutaname <<". Its effect is that the BER decreases by 10 percent each time you receive a mutation, but Mutation Number 3, Strain Evolution, will increase BER only from 7 to 17. "<<endl;
				BER_rise_min=7;
				BER_rise_max=17;
			...
			cout<<"You have "<<mcrb.BER<<" turns before eradication."<<endl;
			sleep(2);
		}
	}
}	
