...
class microbe
{
	...
	void mutate(int BMR,int BER)
	{
		  ...
		  int BCR = 5;
		    ...
		    cout << " Rate of country colonization is now " <<BCR<< ". "<<endl;
...
class place
{
	...
	int assignbcr(microbe mcrb,place p); //ncqr is a global/in scope int variable wrt to the object denoting how many biomes have been conquered 
...
int countdn(microbe mcrb,int ct[])
{
	Biome[ct[0]][ct[1]].assignbcr(mcrb,Biome[ct[0]][ct[1]]);
	while(mcrb.BER>0)
	{
		cout<<"BCR - "<<Biome[ct[0]][ct[1]].BCR;		
		Biome[ct[0]][ct[1]].BCR--;
		mcrb.BER--;
		mcrb.BMR--;
		if(mcrb.BMR==0)
...
int place::assignbcr(microbe mcrb,place p)
{
		p.BCR=5;
		if(mcrb.BER==200)
		Oceania(ct,mcrb);
