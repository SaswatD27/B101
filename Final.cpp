...
int countdn(microbe *micro, place *current)
{
	...
	while(micro->BER!=0)
	{
		...
		micro->BMR--;
		if(micro->BMR==0)
		micro->mutate(micro->BMR,micro->BER);//call functions and code that runs for each step here and modify the parameters of this function as necessary
		if(current->BCR==0)
		{
...
void Asia(microbe *micro)
{
	...
	micro->mutate(micro->BMR,micro->BER);//whatever you have named the mutation choice function
}
